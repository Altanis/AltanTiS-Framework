...
class ExtendedClient extends discord_js_1.Client {
    constructor(options) {
        ...
        if (!this.prefix)
            log.severe('No prefix was provided into the client options.', {
                throw: true
            });
        if (!this.token)
            log.severe('No token was provided into the client options.', {
                throw: true
            });
        this.once('ready', () => {
            if (this.user && !this.user.bot)
                return log.severe('AltaFramework does not support user bots. Please retry with a bot token.', {
                    throw: true
                });
            ...
            this.setInterval(() => {
                ...
                for (let [k, value] of this.cooldown) {
                    ...
                    keys.map(key => {
                        ...
                        if (baseCooldown <= 150)
                            return log.severe('Sorry, but cooldowns must be above 150 milliseconds for AltaFramework to process it. You inputted ' + baseCooldown + ' milliseconds.', {
                                throw: false
                            });
